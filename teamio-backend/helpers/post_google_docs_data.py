...
def post_docs_to_db(doc_json: Dict[str, Any], team_id: str):
    # Import the correct processing functions that handle server format
    import sys
    import os
    sys.path.append(os.path.join(os.path.dirname(__file__), '..', 'scripts'))
    from post_docs_data import process_docs_revisions, process_docs_comments
    
    print(f"Processing Google Docs data for team {team_id}")
    print(f"Document structure keys: {list(doc_json.keys())}")
    
    # Process document data into contributions using the correct functions
