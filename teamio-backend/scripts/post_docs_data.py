...
def _author_to_email(author: str | None) -> str:
    """
    Convert author field to email format.
    Returns the author as-is if it looks like an email, otherwise returns a default.
    """
    if not author:
...
def _email_to_net_id(email: str) -> str:
    # Extract net_id from email (part before @) for consistency with GitHub data
