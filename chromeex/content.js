...
(function () {
  ...
  // Calculate net contributions (final text each author contributed)
  const finalDocText = docText();
  
  // Count characters in final document by author for net contributions
  const netContributionsByUser = {};
  for (const char of docChars) {
    const authorId = char.author;
    netContributionsByUser[authorId] ??= {
      author: char.author,
      netChars: 0,
      netWords: 0,
      netText: ''
    };
    netContributionsByUser[authorId].netChars++;
  }
  
  // Calculate net words and extract net text for each author
  for (const [authorId, stats] of Object.entries(netContributionsByUser)) {
    // Extract only the text this author contributed in the final document
    const authorChars = docChars.filter(c => c.author === authorId);
    const netText = authorChars.map(c => c.ch).join('');
    stats.netText = netText;
    stats.netWords = countWords(netText);
  }

  // Optional: coalesce tiles into larger blocks
  const tilesMerged = coalesceInsertTiles(tilesOut, deletions, {
    maxGapMs: 10 * 60 * 1000,  // 10 minutes instead of 5
    maxIndexDistance: 500,      // 500 characters instead of 200
    allowInterveningDeletions: true
  });

  // Update tile segments to show final text that remained in document
  for (const tile of tilesMerged) {
    const authorId = tile.authorId;
    
    // Try to find the author by name if ID doesn't match
    let finalTextForAuthor = netContributionsByUser[authorId]?.netText || '';
    
    // If not found by ID, try to find by author name
    if (!finalTextForAuthor) {
      for (const [id, netStats] of Object.entries(netContributionsByUser)) {
        if (netStats.author === tile.author) {
          finalTextForAuthor = netStats.netText;
          break;
        }
      }
    }
    
    // Calculate what text from this specific tile actually survived
    const originalTileText = tile.text;
    let survivedText = '';
    
    if (finalTextForAuthor.includes(originalTileText)) {
      // The entire tile text survived
      survivedText = originalTileText;
    } else {
      // Find what parts of this tile's text survived
      const normFinal = norm(finalTextForAuthor);
      const normOriginal = norm(originalTileText);
      
      if (normFinal.includes(normOriginal)) {
        survivedText = originalTileText;
      } else {
        // Find the longest substring that survived
        let longest = '';
        for (let i = 0; i < originalTileText.length; i++) {
          for (let j = i + 1; j <= originalTileText.length; j++) {
            const substring = originalTileText.slice(i, j);
            if (substring.length > longest.length && normFinal.includes(norm(substring))) {
              longest = substring;
            }
          }
        }
        survivedText = longest;
      }
    }
    
    // Store the original text before updating
    tile.originalText = originalTileText;
    
    // Update the tile's main text to show what survived
    tile.text = survivedText;
    
    // Update each segment's text to show what actually remained
    for (const segment of tile.segments) {
      const originalSegmentText = segment.text;
      
      // Store original segment text
      segment.originalText = originalSegmentText;
      
      // Check if this segment's text appears in the survived text
      if (survivedText.includes(originalSegmentText)) {
        // The segment text survived intact
        segment.text = originalSegmentText;
      } else {
        // The segment text was modified or deleted
        segment.text = ''; // Empty if nothing from this segment survived
      }
    }
  }

  // Rebuild per-user totals after coalescing and updating segments
  ...
  // Replace totalWords and totalChars with net contribution values, add totalText
  for (const [authorId, netStats] of Object.entries(netContributionsByUser)) {
    if (totalsByUserMerged[authorId]) {
      totalsByUserMerged[authorId].totalWords = netStats.netWords;
      totalsByUserMerged[authorId].totalChars = netStats.netChars;
      totalsByUserMerged[authorId].totalText = netStats.netText;
    }
  }
  ...
  return {
    ...
    blocks: tilesMerged,
    finalDocumentText: finalDocText
  };
}
