...
const MappingLoginsPage = () => {
  ...
  useEffect(() => {
    ...
    const fetchLogins = async (parsedUserData) => {
      setLoading(true);
      try {
        const response = await fetch(`http://localhost:3000/api/teams/logins?team_id=${parsedUserData.team_id}`);
        if (response.ok) {
          const data = await response.json();
          console.log("Fetched Logins:", data);
          console.log("Team ID:", parsedUserData.team_id);
          setLogins(data || []);
        } else {
    ...
    const fetchMembers = async (parsedUserData) => {
      ...
      try {
        const response = await fetch(`http://localhost:3000/api/teams/members?team_id=${parsedUserData.team_id}`);
        if (response.ok) {
          const data = await response.json();
    ...
    fetchLogins(parsedUserData);
    fetchMembers(parsedUserData);
  }, [teamId]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        navigate("/teamio"); // Navigate to a success page or another route
      } else {
        const errorData = await response.json();
        setResponseMessage("Error: " + JSON.stringify(errorData));
      }
