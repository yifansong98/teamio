...
const GoogleDocsStepper = () => {
  ...
  useEffect(() => {
    if (teamId) {
      console.log("GoogleDocsStepper - Using global teamId:", teamId);
      // Reset data when component mounts with a new teamId
      setCurrentStep(1);
      setGoogleToken(null);
  ...
  const initiateLogin = async () => {
    ...
    try {
      // First, try a status check to see if we're already authenticated
      const statusResponse = await fetch('http://localhost:8787/api/replay', {
        method: 'POST',
        headers: {
      ...
      if (statusResponse.ok) {
        ...
        setError(null);
        setSuccess('Already authenticated with Google! Moving to next step...');
      ...
      // If not authenticated, open browser for manual login
      ...
      // This should open a browser window for manual login
      setError('Browser window opened for authentication. Please sign in to your Google account in the browser window, then use "Check Status" to verify.');
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorData.error && errorData.error.includes('Not signed into Google')) {
          setError('Not authenticated with Google. Please use "Login" button first to authenticate in the browser window.');
        } else {
  ...
  return (
    <div className="p-4 md:p-8">
      ...
      <div className="text-center mb-8">
        <button
          onClick={() => navigate("/home", { state: { teamId: teamId } })}
                ...
                <div className="space-y-4">
                  ...
                  <div className="text-center space-y-4">
                    ...
                    <p className="text-sm text-gray-600">
                      First click "Login with Google" to open a browser window and sign in, then click "Check Status" to verify.
                    </p>
                  </div>
