...
const AnnotateContributionsPage = () => {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const processedContributions = contributionsData.map(c => {
          ...
          const userData = mappedUsers[c.author];
          const mappedNetId = userData?.user_id || c.author;
          return {
            ...c,
            net_id: mappedNetId,
        ...
        const members = {};
        Object.values(mappedUsers).forEach((userData, i) => {
          if (userData && userData.user_id) {
            const netIdStr = String(userData.user_id);
            if (netIdStr && !members[netIdStr]) {
              members[netIdStr] = { 
                net_id: netIdStr, 
                initials: netIdStr.charAt(0).toUpperCase(),
        ...
        const sortedNetIds = Object.values(mappedUsers)
          .map(userData => userData?.user_id)
          .filter(id => id)
          .map(id => String(id))
          .sort();
  ...
  return (
    <div className="p-4 md:p-8 max-w-6xl mx-auto">
      ...
      <h1 className="text-2xl font-bold text-gray-800">Step 3: Annotate Contributions</h1>
                      ...
                      <div className="w-full">
                        <div className="bg-white p-4">
                          <div className="text-sm text-gray-700">
                            ...
                            {(() => {
                              // Complete text reconstruction algorithm from the commit
                              let tileText = c.text || '';
                              ...
                              // Additional processing for Google Docs tiles
                              if (c.googleTile) {
                                // Handle nested tiles if present
                                if (c.nestedTiles && c.nestedTiles.length > 0) {
                                  const nestedText = c.nestedTiles
                                    .map(nested => nested.text || '')
                                    .filter(text => text.trim())
                                    .join(' ');
                                  tileText = tileText + (nestedText ? ' ' + nestedText : '');
                                }
                              ...
                              return tileText.length > 0
                                ? <pre className="whitespace-pre-wrap font-sans text-gray-800 w-full">{tileText}</pre>
